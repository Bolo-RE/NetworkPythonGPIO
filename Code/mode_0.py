...
def mode_0():  # création de la fonction mode0
    while 1:  # loop infinie
        if (
            gpio.readpin(12) is True
        ):  # si le bouton poussoir est relâché, set la valeur du pin a 1
            gpio.setpin(24, 1)  # la LED est eteinte
        else:
            gpio.setpin(
                24, 0
            )  # si la condition n'est pas remplie par le booléen précedent, le BP est enfoncé
            # la LED est donc allumée car pin_value vaut 0
        if gpio.readpin(16) is True:
            gpio.setpin(25, 1)
        else:
            gpio.setpin(25, 0)
        if gpio.readpin(20) is True:
            gpio.setpin(7, 1)
        else:
            gpio.setpin(7, 0)
        if gpio.readpin(21) is True:
            gpio.setpin(8, 1)
        else:
            gpio.setpin(8, 0)
